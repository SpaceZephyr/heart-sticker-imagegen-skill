...
当前 Agent 没有生图能力，或生成效果不理想时，发送：

```text
生图 API
```

这两个关键词都会返回心念贴纸客服入口，由小助理继续协助打印或 API 接入。
...
- 发送“打印贴纸”，获取实体贴纸打印与邮寄服务入口。
- 发送“生图 API”，获取没有可靠生图能力时的 API 接入服务入口。
...
- 打印和生图 API 入口只返回客服联系方式；Skill 不提供价格承诺，不自动下单，也不会在对话中公开 API key。
